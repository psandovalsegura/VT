# coding=utf-8
"""Implementation of VNI-FGSM attack."""
...
def graph(x, y, i, x_max, x_min, grad, variance):
    ...
    # noise = new_grad / tf.reduce_mean(tf.abs(new_grad), [1, 2, 3], keep_dims=True)
...
def main(_):
    ...
    with tf.Graph().as_default():
        ...
        #     cross_entropy = tf.losses.softmax_cross_entropy(one_hot, logits_v3)
        #     grad += tf.gradients(cross_entropy, x_input)[0]

        # iter = tf.constant(0)
        # max_iter = tf.constant(100)
        # _, _, _, _, _, global_grad = tf.while_loop(grad_finish, batch_grad, [x_input, one_hot, iter, max_iter, FLAGS.max_epsilon / 510.0, grad])

        x_adv, _, _, _, _, _, _ = tf.while_loop(stop, graph, [x_input, y, i, x_max, x_min, tf.zeros(shape=batch_shape), tf.zeros(shape=batch_shape)])
